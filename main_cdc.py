...
parser.add_argument('--list-classes', default=['Normal', 'WDEA', 'MDEA', 'LDEA', 'SEIC'], type=list,
...
parser.add_argument('--checkpoint', default='./checkpoint-cdc/checkpoint',
                    help='Path where save the model checkpoint')
parser.add_argument('--checkpoint-cluster', default='./checkpoint-cdc/checkpoint-cluster',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, queue, criterion, optimizer, epoch, args)
...
def train(train_loader, model, queue, criterion, optimizer, epoch, args):
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        cluster_labels = queue.update(z, labels)

        loss = criterion(output, cluster_labels)
